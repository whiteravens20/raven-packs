...
// The check runs once, when the block is placed. Nothing runs per tick.// Wrapped in a function because KubeJS runs every server script in one shared
...
(function () {
  ...
  BlockEvents.placed(event => {
    ...
    if (areaIsClear(event.server, player.uuid, event.level.dimension, block.x, block.z, radius)) return
  ...
  BlockEvents.rightClicked(event => {
    ...
    const player = event.player
    if (player != null && isOwnClaim(event.server, player.uuid, event.level.dimension, target.x, target.z)) return
